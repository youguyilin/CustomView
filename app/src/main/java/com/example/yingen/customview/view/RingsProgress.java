...
import com.example.yingen.customview.utils.DensityUtils;
...
public class RingsProgress extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private Paint paint;
    private List<Rings> Ringss = new ArrayList<>();
     ...
     * 旋转展现动画
     */
    private ValueAnimator RingsAnimator;
    ...
    public RingsProgress(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    public void setData(List<Rings> cakes) {
        if (null != Ringss) {
            // 初始化Ringss;
            ...
            for (int i = 0; i < cakes.size(); i++) {
                if (cakes.get(i).getItemValue() > 0) {
                    ...
                    Ringss.add(new Rings(cakes.get(i).getItemType(), value,
                            cakes.get(i).getColors()));

                }
            }
            if (Ringss.size() == 0) {
                Ringss.add(new Rings("", 100, defaultColor));
            }
            settleRingss(Ringss.size() - 1);
            // 初始化itemframe
            itemFrame = new float[Ringss.size()];
            for (int i = 0; i < Ringss.size(); i++) {
                if (i == 0) {
                    itemFrame[i] = (float) Ringss.get(i).getItemValue();
                    continue;
                }
                itemFrame[i] = (float) Ringss.get(i).getItemValue()
                        + itemFrame[i - 1];
    ...
    private double getSum(List<Rings> mCakes) {
        double sum = 0;
    ...
    private float getSum(List<Rings> mCakes, int index) {
        float sum = 0;
    ...
    /**
     * 使用递归保证Ringss的值的总和必为100
     *
     * @param i
     */
    private void settleRingss(int i) {
        float sum = getSum(Ringss, i);
        Rings value = Ringss.get(i);
        if (sum <= 100f) {
    ...
    private void drawCake() {
        ...
        if (null != canvas) {
            ...
            for (int i = 0; i < Ringss.size(); i++) {
                paint.setColor(Color.parseColor(Ringss.get(i).getColors()));
                if (i == 0) {
                    canvas.drawArc(pieOval, 0,
                            (float) Ringss.get(i).getItemValue() * ANGLE_NUM, true,
                            paint);
                    continue;
                }
                canvas.drawArc(pieOval, itemFrame[i - 1]
                        * ANGLE_NUM, (float) Ringss.get(i).getItemValue()
    ...
    private void drawArc() {
        ...
        if (null != canvas) {
            ...
            int colorIndex = curItem % Ringss.size();
            if (curItem == itemFrame.length - 1 && colorIndex == 0) {
                colorIndex = 0;
            }
            paint.setColor(Color.parseColor(Ringss.get(colorIndex).getColors()));
    ...
    private void initValueAnimator() {
        ...
        PropertyValuesHolder angleValues = PropertyValuesHolder.ofFloat(
                "angle", 0f, 360f);
        RingsAnimator = ValueAnimator.ofPropertyValuesHolder(angleValues);
        RingsAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
        ...
        RingsAnimator.setDuration(duration);
        RingsAnimator.setRepeatCount(0);
        RingsAnimator.setInterpolator(new DecelerateInterpolator());
        RingsAnimator.setRepeatMode(ValueAnimator.RESTART);
    }
